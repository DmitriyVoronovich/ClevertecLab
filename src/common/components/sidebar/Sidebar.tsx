...
export const Sidebar = ({handleOpen, open}: SidebarProps) => {
    ...
    const menu = NavigationMenuData.map((item) => {
        ...
        return (
            <div className={'sidebar_menu_item'} key={item.id} onClick={onClickHandler}
                ...
                <span
                    className={`${open ? 'sidebar_item_title' : 'sidebar_item_title close'}`}>{item.title}</span>
            </div>
        )
    })
    ...
    return (
        <div
            className={`${open ? 'sidebar_content_container' : 'sidebar_content_container close_menu'}`}>
            <div
                className={`${open ? 'sidebar_menu_wrapper' : 'sidebar_menu_wrapper close_menu_item'}`}>
                <div
                    className={`${open ? 'sidebar_logo_container' : 'sidebar_logo_container close_logo_item'}`}>
                    <Logo open={open}/>
                </div>
            ...
            <div className={`${open ? 'sidebar_footer' : 'sidebar_footer close_menu_item'}`}
                ...
                <span
                    className={`${open ? 'sidebar_footer_text' : 'sidebar_footer_text close'}`}>Выход</span>
            </div>
