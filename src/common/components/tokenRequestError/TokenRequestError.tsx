...
export const TokenRequestError = ({callback, status}: TokenRequestErrorProps) => {
    ...
    return (
        <Modal className={'error_request_container'} open={isModalOpen} closable={false}
               footer={null} data-test-id='modal-no-review'>
            <div className={'error_request_wrapper'}>
                ...
                <Button type="primary" className={'error_request_button'}
                        onClick={onCancelModalForm}>Назад</Button>
            </div>
        </Modal>
    );
};
